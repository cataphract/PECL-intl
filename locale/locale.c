...
void locale_register_constants( INIT_FUNC_ARGS )
{
	...
	#define LOCALE_EXPOSE_CUSTOM_CLASS_CONST(name, value) zend_declare_class_constant_long( Locale_ce_ptr, ZEND_STRS( name ) - 1, value TSRMLS_CC );

	LOCALE_EXPOSE_CLASS_CONST( ACTUAL_LOCALE );
	LOCALE_EXPOSE_CLASS_CONST( VALID_LOCALE );

	LOCALE_EXPOSE_CUSTOM_CLASS_CONST( "OPTION_DEFAULT", LOC_DEFAULT );
